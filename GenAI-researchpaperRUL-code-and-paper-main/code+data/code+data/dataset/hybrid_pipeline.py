...
SEQ_LEN = 96
PRED_LEN = 1
LAST_N_ROWS = _env_int("LAST_N_ROWS", 3000)
TRAIN_RATIO = 0.80
VAL_RATIO = 0.0
TEST_RATIO = 0.20
...
USE_CHRONOS_ONLY = not _env_bool("HYBRID_MODE", False)
...
class PatchTSTResidual(nn.Module):
    def __init__(self, input_dim, seq_len, d_model=64, n_heads=4, n_layers=2, dropout=0.1):
        super().__init__()
        ...
        layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=n_heads, dim_feedforward=d_model*2, batch_first=True, dropout=dropout)
...
class TinyTimeMixer(nn.Module):
    def __init__(self, input_dim, seq_len, hidden=64, dropout=0.1):
        super().__init__()
        self.net = nn.Sequential(nn.Flatten(), nn.Linear(input_dim*seq_len + 2, hidden*2), nn.ReLU(), nn.Dropout(dropout), nn.Linear(hidden*2, hidden), nn.ReLU(), nn.Linear(hidden, 1))
...
def train_model(model, train_loader, val_loader, epochs=200, patience=15, threshold=None, lr=1e-3):
    """
...
def main():
    ...
    dropout_val = float(os.environ.get("DROPOUT", "0.1"))
    lr_val = float(os.environ.get("LEARNING_RATE", "1e-3"))
    epochs_val = int(os.environ.get("EPOCHS", "200"))

    m1 = PatchTSTResidual(X_tr.shape[2], SEQ_LEN, dropout=dropout_val)
    m2 = TinyTimeMixer(X_tr.shape[2], SEQ_LEN, dropout=dropout_val)
    CONSOLE.print(f"\n[bold yellow]Training models with benchmark target: {benchmark_mae}[/bold yellow]")
    CONSOLE.print(f"[dim]Hyperparameters: Batch={BATCH_SIZE} LR={lr_val} Epochs={epochs_val} Dropout={dropout_val}[/dim]")
    
    disable_early_stopping = _env_bool("DISABLE_EARLY_STOPPING", False)
    train_threshold = None if disable_early_stopping else benchmark_mae
    
    train_model(m1, tr_loader, val_loader, epochs=epochs_val, threshold=train_threshold, lr=lr_val)
    train_model(m2, tr_loader, val_loader, epochs=epochs_val, threshold=train_threshold, lr=lr_val)
