...
latent_dim = 4 # Increased latent dim from 2 to 4 to capture more morphological variance (low JS div)
...
vae = VAE(encoder, decoder, beta=0.8) # Moderate beta: lower beta improves reconstruction (Wasserstein/JS) at cost of KL
vae.compile(optimizer=keras.optimizers.Adam(learning_rate=0.002))

print("\n--- Training VAE for strict Divergence Bounds ---")
vae.fit(features_scaled, epochs=250, batch_size=32, verbose=2)

# 3. Evaluation & Generation Loop
...
print(f" - Average Wasserstein: {avg_wd:.4f} (Target < 0.1)")
print(f" - Average JS Divergence: {avg_js:.4f} (Target < 0.4)")
...
out_file = os.path.join(base_dir, "synthetic_battery_cycles_50k.csv")
